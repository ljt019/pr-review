import os

from dotenv import load_dotenv

from agent.agent import ModelOptions, SniffAgent
from agent.messages import BugReportMessage, ToolCallMessage, ToolResultMessage
from paths import get_assets_path

# Load environment variables from .env file
load_dotenv()

# Enable verbose container debug logs for every run
os.environ["SNIFF_DEBUG"] = "0"

zipped_codebase_path = get_assets_path("toy-webserver.zip")


def main():
    try:
        bot = SniffAgent(zipped_codebase_path, ModelOptions.QWEN3_30B_A3B_INSTRUCT)
        print("[DEBUG] Using run_streaming() to debug tool calls...")
        for message in bot.run_streaming():
            print(f"[DEBUG] Got streaming message: {type(message).__name__}")
            if isinstance(message, ToolCallMessage):
                print(
                    f"[DEBUG] Tool call: {message.tool_name}, Args: {message.arguments}"
                )
            elif isinstance(message, ToolResultMessage):
                print(
                    f"[DEBUG] Tool result: {message.tool_name}, Output: {message.result}"
                )
            elif isinstance(message, BugReportMessage):
                print(f"[DEBUG] Final report content:\n{message.content}")
                break
    except Exception as e:
        print(f"Error: {e}")
    finally:
        if "bot" in locals():
            bot._cleanup()


if __name__ == "__main__":
    main()
