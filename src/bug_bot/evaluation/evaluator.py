...
class BugDetectionEvaluator:
    ...
    def _wilson_confidence_interval(
        self, successes: int, total: int, z: float = 1.96
    ) -> Tuple[float, float]:
    ...
    def format_report(self, evaluation_results: Dict) -> str:
        ...
        report.append("\n📊 SUMMARY METRICS:")
        report.append(
            f"  Total Ground Truth Bugs: {summary['total_ground_truth_bugs']}"
        )
        ...
        precision_ci = (
            self._wilson_confidence_interval(tp, total_detections)
            if total_detections > 0
            else (0, 0)
        )
        recall_ci = (
            self._wilson_confidence_interval(tp, total_truth)
            if total_truth > 0
            else (0, 0)
        )
        ...
        report.append(
            f"  🎯 Recall: {summary['recall']:.2%} (95% CI: {recall_ci[0]:.2%}-{recall_ci[1]:.2%})"
        )
        ...
        if total_truth < 20:
            report.append(
                f"  ⚠️  Small sample size (n={total_truth}) - results may have high uncertainty"
            )
        if summary["precision"] < 0.7 or summary["recall"] < 0.3:
            report.append(
                "  ⚠️  Performance concerns detected - consider ground truth review"
            )
        ...
        for severity in ["critical", "major", "minor"]:
            ...
            report.append(
                f"  {severity.upper()}: {data['found']}/{data['total']} found (Recall: {data['recall']:.2%})"
            )

        # Performance by category
        report.append("\n📂 PERFORMANCE BY CATEGORY:")
        sorted_categories = sorted(
            evaluation_results["by_category"].items(),
            key=lambda x: x[1]["recall"],
            reverse=True,
        )
        for category, data in sorted_categories:
            report.append(
                f"  {category}: {data['found']}/{data['total']} found (Recall: {data['recall']:.2%})"
            )
        ...
        if evaluation_results["matches"]:
            ...
            for match in evaluation_results["matches"][:3]:
                ...
                report.append(
                    f"  Matched to: {match['truth'].get('id', 'Unknown')} - {match['truth'].get('description', '')[:60]}..."
                )
        ...
        critical_missed = [
            bug
            for bug in evaluation_results["missed_bugs"]
        ...
        if evaluation_results["false_positives"]:
            ...
            for bug in evaluation_results["false_positives"][:3]:
                ...
                report.append(
                    f"    File: {bug.get('file', 'Unknown')}, Line: {bug.get('line', 'Unknown')}"
                )
