...
def generate_metadata(
    ...
    metadata = {
        ...
        "files_with_bugs": list(
            set(bug.get("file", "") for bug in bugs if bug.get("file"))
        ),
        "analysis_summary": {
            "total_files": len(files_analyzed),
            "files_with_issues": len(
                set(bug.get("file", "") for bug in bugs + nitpicks if bug.get("file"))
            ),
            "most_problematic_category": category_counts.most_common(1)[0][0]
            if category_counts
            else None,
        },
    }
...
def _calculate_confidence(
    bugs: List[Dict[str, Any]], nitpicks: List[Dict[str, Any]]
) -> float:
    """
