...
class GrepTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        try:
            ...
            return self._ripgrep_search(pattern, directory, include_files, exclude_files,
                                      case_sensitive, use_regex, max_results, context_lines)
                
        except Exception as e:
            return f"Error: {str(e)}"

    def _ripgrep_search(self, pattern: str, directory: str = '.', include_files: str = None,
                       exclude_files: str = None, case_sensitive: bool = False, 
                       use_regex: bool = False, max_results: int = 100, 
        ...
        if include_files:
            if include_files.startswith('*.'):
                ...
                ext = include_files[2:]
                if ext in ['py', 'js', 'ts', 'java', 'cpp', 'c', 'go', 'rs', 'rb', 'php', 'sh']:
                    cmd.extend(['--type', ext])
                else:
