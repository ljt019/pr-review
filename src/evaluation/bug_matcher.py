...
class SemanticBugMatcher:
    ...
    def __init__(self, embedding_model: str = 'all-MiniLM-L6-v2'):
        ...
        self.category_patterns = {
            'performance': [
                r'rate.{0,20}limit(?:ing)?', r'timeout', r'batch.{0,20}(?:operation|query)',
                r'inefficient', r'n\+1', r'optimization', r'slow.{0,20}query',
                r'memory.{0,20}leak', r'high.{0,20}cpu', r'per.{0,20}request',
                r'loop.{0,20}(?:per|each)', r'cache.{0,20}(?:miss|invalidat)',
                r'(?:database|db).{0,20}(?:call|query).*(?:loop|each)',
                r'automatic.{0,20}save', r'frequent.{0,20}(?:save|write)'
            ],
            'validation': [
                r'input.{0,20}validation', r'sanitiz', r'file.{0,20}size',
                r'type.{0,20}check', r'format.{0,20}validation', r'bounds.{0,20}check',
                r'length.{0,20}validation', r'whitelist', r'blacklist',
                r'no.{0,20}validation', r'(?:size|type).{0,20}validation',
                r'path.{0,20}validation', r'email.{0,20}validation',
                r'backup.{0,20}(?:path|file)', r'config.{0,20}validation'
            ],
            'error_handling': [
                r'exception.{0,20}handling', r'try.{0,20}catch', r'error.{0,20}suppress',
                r'silent.{0,20}fail', r'broad.{0,20}except', r'bare.{0,20}except',
                r'error.{0,20}handling', r'fault.{0,20}tolerance',
                r'(?:except|catch).*(?:pass|ignore)', r'\bpass\b.*exception',
                r'swallow(?:ed|ing)', r'missing.{0,20}error'
            ],
            'resource_management': [
                r'resource.{0,20}leak', r'file.{0,20}handle', r'connection.{0,20}pool',
                r'memory.{0,20}management', r'garbage.{0,20}collection', r'cleanup',
                r'(?:connection|socket|file).{0,20}(?:not.{0,20})?clos(?:e|ed|ing)',
                r'(?:resource|handle).{0,20}cleanup', r'(?:set|assign).{0,20}none'
            ],
            'authorization': [
                r'access.{0,20}control', r'permission.{0,20}check', r'privilege.{0,20}escalation',
                r'authorization', r'rbac', r'role.{0,20}based',
                r'(?:missing|no).{0,20}(?:authorization|permission)',
                r'requesting.{0,20}user', r'verify.{0,20}permission'
            ],
            'reliability': [
                r'timeout', r'hang(?:ing)?', r'request.{0,20}timeout',
                r'(?:connection|network).{0,20}(?:timeout|hang)',
                r'prevent.{0,20}hanging', r'reliability', r'fault.{0,20}tolerance'
            ],
            'dead_code': [
                r'dead.{0,20}code', r'unused', r'unreachable',
                r'never.{0,20}(?:used|called|reached)', r'obsolete',
                r'(?:import|function|method).{0,20}(?:not|never).{0,20}used',
                r'after.{0,20}return', r'unreachable.{0,20}code'
            ]
        }
    ...
    def normalize_security_terms(self, text: str, term_mappings: Dict[str, List[str]]) -> str:
    ...
    def semantic_similarity(self, detected: Dict, truth: Dict) -> float:
        ...
        detected_text = ' '.join(filter(None, [
            detected.get('title', ''),
    ...
    def pattern_similarity(self, detected: Dict, truth: Dict) -> float:
        ...
        detected_text = (detected.get('description', '') + ' ' + 
                        detected.get('title', '')).lower()
        truth_text = (truth.get('description', '') + ' ' + 
                     truth.get('code_snippet', '')).lower()
    ...
    def calculate_match_score(self, detected: Dict, truth: Dict) -> Tuple[float, List[str]]:
    ...
    def get_adaptive_threshold(self, truth_bug: Dict) -> float:
        ...
        base_threshold = category_thresholds.get(category, 0.60)  # Lower default threshold
    ...
    def match_bugs(self, detected_bugs: List[Dict], ground_truth_bugs: List[Dict], 
        ...
        while True:
            ...
            for i in range(len(detected_bugs)):
                ...
                for j in range(len(ground_truth_bugs)):
                    ...
                    adaptive_threshold = self.get_adaptive_threshold(ground_truth_bugs[j])
